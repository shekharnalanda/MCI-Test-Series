...
class WikidataUniversityCountryImporter
{
    ...
    public function import(int $limit = 500, bool $dryRun = false): array
    {
        ...
        $facts = collect($response->json('results.bindings', []))->map(fn (array $row) => $this->fact($row))->filter()
            ->groupBy('university_url')->filter(fn (Collection $rows) => $rows->unique('country_url')->count() === 1)
            ->map(fn (Collection $rows) => $rows->first())->values();
        $countries = $facts->unique('country_url')->values();
        if ($facts->count() < 4 || $countries->count() < 4) {
            throw new RuntimeException('At least four unambiguous bilingual universities and distinct countries are required.');
        }
        ...
        $questions = $facts->map(function (array $fact) use ($countries, $subject, $topic, $examIds): array {
            $options = $countries->reject(fn (array $candidate) => $candidate['country_url'] === $fact['country_url'])
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['university_url'].'|'.$candidate['country_url']))->take(3)->push($fact)
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['university_url'].'|option|'.$candidate['country_url']))->values()
            ...
            return [
                'question_text' => "In which country is {$fact['university_en']} located?",
                'question_text_hi' => "{$fact['university_hi']} किस देश में स्थित है?",
                'explanation' => "{$fact['university_en']} is located in {$fact['country_en']}.",
                'explanation_hi' => "{$fact['university_hi']} {$fact['country_hi']} में स्थित है।",
                'subject_id' => $subject->id, 'topic_id' => $topic->id, 'exam_ids' => $examIds,
                'difficulty' => 'medium', 'language' => 'bilingual', 'source_url' => $fact['university_url'],
                'source_reference' => 'wikidata-university-country', 'source_published_at' => now()->toDateString(),
    ...
    {
        $fact = ['university_url' => data_get($row, 'university.value'), 'university_en' => data_get($row, 'universityLabelEn.value'),
            'university_hi' => data_get($row, 'universityLabelHi.value'), 'country_url' => data_get($row, 'country.value'),
    ...
    {
        return <<<SPARQL
SELECT DISTINCT ?university ?universityLabelEn ?universityLabelHi ?country ?countryLabelEn ?countryLabelHi WHERE {
  ?university wdt:P31 wd:Q3918;
              wdt:P17 ?country;
              rdfs:label ?universityLabelEn;
              rdfs:label ?universityLabelHi.
  ?country rdfs:label ?countryLabelEn;
           rdfs:label ?countryLabelHi.
  FILTER(LANG(?universityLabelEn) = "en")
  FILTER(LANG(?universityLabelHi) = "hi")
  FILTER(LANG(?countryLabelEn) = "en")
  FILTER(LANG(?countryLabelHi) = "hi")
}
ORDER BY ?universityLabelEn ?countryLabelEn
LIMIT {$limit}
SPARQL;
    }
}
