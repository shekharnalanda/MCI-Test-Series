...
class WikidataAirportCountryImporter
{
    ...
    public function import(int $limit = 500, bool $dryRun = false): array
    {
        ...
        $facts = collect($response->json('results.bindings', []))->map(fn (array $row) => $this->fact($row))->filter()
            ->groupBy('airport_url')->filter(fn (Collection $rows) => $rows->unique('country_url')->count() === 1
                && $rows->unique('iata')->count() === 1)
            ->map(fn (Collection $rows) => $rows->first())->values();
        $countries = $facts->unique('country_url')->values();
        if ($facts->count() < 4 || $countries->count() < 4) {
            throw new RuntimeException('At least four unambiguous bilingual airports and distinct countries are required.');
        }
        ...
        $questions = $facts->map(function (array $fact) use ($countries, $subject, $topic, $examIds): array {
            $options = $countries->reject(fn (array $candidate) => $candidate['country_url'] === $fact['country_url'])
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['airport_url'].'|'.$candidate['country_url']))->take(3)->push($fact)
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['airport_url'].'|option|'.$candidate['country_url']))->values()
            ...
            return [
                'question_text' => "In which country is {$fact['airport_en']} ({$fact['iata']}) located?",
                'question_text_hi' => "{$fact['airport_hi']} ({$fact['iata']}) किस देश में स्थित है?",
                'explanation' => "{$fact['airport_en']} ({$fact['iata']}) is located in {$fact['country_en']}.",
                'explanation_hi' => "{$fact['airport_hi']} ({$fact['iata']}) {$fact['country_hi']} में स्थित है।",
                'subject_id' => $subject->id, 'topic_id' => $topic->id, 'exam_ids' => $examIds,
                'difficulty' => 'medium', 'language' => 'bilingual', 'source_url' => $fact['airport_url'],
                'source_reference' => 'wikidata-airport-country', 'source_published_at' => now()->toDateString(),
    ...
    {
        $fact = ['airport_url' => data_get($row, 'airport.value'), 'airport_en' => data_get($row, 'airportLabelEn.value'),
            'airport_hi' => data_get($row, 'airportLabelHi.value'), 'country_url' => data_get($row, 'country.value'),
            'country_en' => data_get($row, 'countryLabelEn.value'), 'country_hi' => data_get($row, 'countryLabelHi.value'),
            'iata' => data_get($row, 'iata.value')];
    ...
    {
        return <<<SPARQL
SELECT DISTINCT ?airport ?airportLabelEn ?airportLabelHi ?country ?countryLabelEn ?countryLabelHi ?iata WHERE {
  ?airport wdt:P31 wd:Q1248784;
           wdt:P238 ?iata;
           wdt:P17 ?country;
           rdfs:label ?airportLabelEn;
           rdfs:label ?airportLabelHi.
  ?country rdfs:label ?countryLabelEn;
           rdfs:label ?countryLabelHi.
  FILTER(LANG(?airportLabelEn) = "en")
  FILTER(LANG(?airportLabelHi) = "hi")
  FILTER(LANG(?countryLabelEn) = "en")
  FILTER(LANG(?countryLabelHi) = "hi")
}
ORDER BY ?airportLabelEn ?countryLabelEn
LIMIT {$limit}
SPARQL;
    }
}
