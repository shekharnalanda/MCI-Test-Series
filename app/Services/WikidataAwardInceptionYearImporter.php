...
class WikidataAwardInceptionYearImporter
{
    ...
    public function import(int $limit = 500, bool $dryRun = false): array
    {
        ...
        $facts = collect($response->json('results.bindings', []))->map(fn (array $row) => $this->fact($row))->filter()
            ->groupBy('award_url')->filter(fn (Collection $rows) => $rows->unique('year')->count() === 1)
            ->map(fn (Collection $rows) => $rows->first())->values();
        $years = $facts->unique('year')->values();
        if ($facts->count() < 4 || $years->count() < 4) {
            throw new RuntimeException('At least four unambiguous bilingual awards and distinct inception years are required.');
        }
        ...
        $questions = $facts->map(function (array $fact) use ($years, $subject, $topic, $examIds): array {
            $options = $years->reject(fn (array $candidate) => $candidate['year'] === $fact['year'])
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['award_url'].'|'.$candidate['year']))->take(3)->push($fact)
                ->sortBy(fn (array $candidate) => hash('sha256', $fact['award_url'].'|option|'.$candidate['year']))->values()
            ...
            return [
                'question_text' => "In which year was {$fact['award_en']} established?",
                'question_text_hi' => "{$fact['award_hi']} की स्थापना किस वर्ष हुई थी?",
                'explanation' => "{$fact['award_en']} was established in {$fact['year']}.",
                'explanation_hi' => "{$fact['award_hi']} की स्थापना {$this->toHindiDigits((string) $fact['year'])} में हुई थी।",
                'subject_id' => $subject->id, 'topic_id' => $topic->id, 'exam_ids' => $examIds,
                'difficulty' => 'medium', 'language' => 'bilingual', 'source_url' => $fact['award_url'],
                'source_reference' => 'wikidata-award-inception-year', 'source_published_at' => now()->toDateString(),
    ...
    {
        $url = data_get($row, 'award.value');
        $nameEn = data_get($row, 'awardLabelEn.value');
        $nameHi = data_get($row, 'awardLabelHi.value');
        $date = data_get($row, 'inception.value');
        if (! collect([$url, $nameEn, $nameHi, $date])->every(fn ($value) => is_string($value) && trim($value) !== '')) {
            return null;
        }
        $year = (int) substr($date, 0, 4);
        if ($year < 1000 || $year > (int) now()->format('Y')) {
            return null;
        }

        return ['award_url' => $url, 'award_en' => $nameEn, 'award_hi' => $nameHi, 'year' => $year];
    }
    ...
    {
        return <<<SPARQL
SELECT DISTINCT ?award ?awardLabelEn ?awardLabelHi ?inception WHERE {
  ?award wdt:P571 ?inception;
         wdt:P31/wdt:P279* wd:Q618779;
         rdfs:label ?awardLabelEn;
         rdfs:label ?awardLabelHi.
  FILTER(LANG(?awardLabelEn) = "en")
  FILTER(LANG(?awardLabelHi) = "hi")
  FILTER(DATATYPE(?inception) = xsd:dateTime)
}
ORDER BY ?awardLabelEn ?inception
LIMIT {$limit}
SPARQL;
    }
}
