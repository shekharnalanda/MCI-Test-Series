...
class WikidataSoftwareChronologyHardImporter
{
    ...
    public function import(int $limit = 15, bool $dryRun = false): array
    {
        ...
        if ($facts->count() < 8) {
            throw new RuntimeException('At least eight verified bilingual software release facts are required.');
        }
    ...
    private function facts(int $sourceId): Collection
    {
        return Question::query()
            ->where('content_source_id', $sourceId)->where('source_reference', 'wikidata-software-first-release-year')
            ...
            ->get()->map(function (Question $question) {
                ...
                if (! $answer || ! preg_match('/^In which year was (.+) first released\?$/u', $question->question_text, $en)
                    || ! preg_match('/^(.+) पहली बार किस वर्ष जारी किया गया था\?$/u', (string) $question->question_text_hi, $hi)
    ...
    private function payload(Collection $group, int $subjectId, int $topicId, array $examIds): array
    {
        ...
        $orders = collect([$ascending, $ascending->reverse()->values(),
        ...
        return [
            'question_text' => "Which option arranges {$namesEn} in increasing order of first release year?",
            'question_text_hi' => "कौन-सा विकल्प {$namesHi} को प्रथम रिलीज वर्ष के बढ़ते क्रम में व्यवस्थित करता है?",
            ...
            'source_reference' => 'wikidata-software-chronology:'.$entities->implode(','),
