...
class WikidataElementOrderHardQuestionImporterTest extends TestCase
{
    ...
    public function test_it_imports_a_distinct_atomic_number_range_reasoning_format(): void
    {
        $this->seed();
        Http::fake([
            'https://www.wikidata.org/*' => Http::response('ok', 200),
        ...
        $this->artisan('mci:wikidata-element-order-hard --format=range --limit=3')->assertSuccessful();

        $questions = Question::where('source_reference', 'like', 'wikidata-element-range:%')->get();
